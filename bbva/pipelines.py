...
class BbvaPipeline:
    ...
    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `bbva`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM bbva WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO bbva (date, title, link, content)"
